      ...
      .then(result => {
        ...
        bcrypt.compare(req.body.password, result.password, function (err, response) {
          if (response) {
            ...
            res.status(400).send('you entered incorrect password');
          }
        });
      }).catch(err => {
        res.status(404).send('you entered incorrect email');
      });
  }
});
...
router.post('/add', (req, res) => {
  ...
  if (validating.error) {
    ...
    user.save()
      .then(result => {
        // check if the user saved in the database
        res.send('You have added a new user');
        console.log(result);
      })
      .catch(err => {
        // check if the user isn't saved in the database
